...
- The `walrus` CLI now supports creating, funding, and extending *shared blobs* using the `walrus
  ...
  example of collectively managed and funded blobs. See the [shared blobs section](/docs/walrus-client/storing-blobs#shared-blobs) for more details.
...
instance. You need to request new WAL tokens through the [Testnet WAL faucet](/docs/system-overview/available-networks#testnet-wal-faucet).
...
use the latest version of the configuration files, see the [configuration section](/docs/getting-started/advanced-setup#configuration).
...
the [JSON API](/docs/walrus-client/json-mode)).
...
also the aggregator and publisher. Please refer to the section on the [HTTP API](/docs/http-api/storing-blobs) for further details.
