...
Open **port 9185** (storage node) and **port 80** (automatic certificate renewal). If you plan to run an [aggregator or publisher](/docs/operator-guide/aggregators/operating-aggregator) on the same host, also open **port 9000** (aggregator) or **port 9001** (publisher), or **port 443** if you use a reverse proxy (recommended for production):
